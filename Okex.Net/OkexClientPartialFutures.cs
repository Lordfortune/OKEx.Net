...
namespace Okex.Net
{
	public partial class OkexClient
	{
		...
		public WebCallResult<OkexSpotPlacedOrder> Futures_PlaceOrder(OkexFuturesOrderParams orderParams,
			CancellationToken ct = default)
		{
			return Futures_PlaceOrder_Async(orderParams, ct).Result;
		}

		public async Task<WebCallResult<OkexSpotPlacedOrder>> Futures_PlaceOrder_Async(OkexFuturesOrderParams orderParams, CancellationToken ct = default)
		{
			orderParams.InstrumentId = orderParams.InstrumentId.ValidateSymbol();

			orderParams.ClientOrderId?.ValidateStringLength("clientOrderId", 0, 32);
			...
			if (orderParams.ClientOrderId != null && !Regex.IsMatch(orderParams.ClientOrderId, "^(([a-z]|[A-Z]|[0-9]){0,32})$"))
			...
			return await SendRequest<OkexSpotPlacedOrder>(GetUrl(Endpoints_Futures_PlaceOrder), HttpMethod.Post, ct, parameters, signed: true).ConfigureAwait(false);
		}
